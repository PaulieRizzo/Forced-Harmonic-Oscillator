...
while time < stop_time:
    ...
    delta_x = x_0 * cos(sqrt(k/m) * time + asin(-v_0 * sqrt(m/k) / x_0))  # Analytic Solution

    acceleration = (eq - ball.pos) * (
            spring.constant / ball.mass)  # a = x * k / kg  (Similar to F=-k*x, although lacks mass)

    ball.pos.x = ball.pos.x - delta_x * time_step - spring.velocity.x * time_step  # Moves ball with spring
...
ax = plt.subplots()
ax = plt.gca()
plt.plot(Time_1, Force_1, label='Mass = 0.5')
plt.plot(Time_2, Force_2, label='Mass = 1.0')
plt.plot(Time_3, Force_3, label='Mass = 2.0')
plt.xlabel('Time')
plt.ylabel('Force')
plt.legend()
plt.show()
